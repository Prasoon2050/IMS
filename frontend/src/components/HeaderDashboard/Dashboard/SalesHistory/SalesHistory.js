...
const SalesHistory = ({ role }) => {
  ...
  const handleSearch = () => {
    if (searchDate) {
      axios
        .get(
          `https://shopsense-api.vercel.app/api/admin/get-bills-by-date?date=${searchDate}`,
          {
            headers: {
  ...
  useEffect(() => {
    axios
      .get("https://shopsense-api.vercel.app/api/admin/get-bills", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
          ...
          <tbody>
            {searchPerformed && filteredBills.length === 0 ? (
              ...
              filteredBills.map((bill, index) => (
                <tr key={index}>
                  <td>{bill.billNo}</td>
                  <td>{new Date(bill.date).toLocaleString("en-GB")}</td>
